...
const displayedUpvotes = computed(
  () => Number(props.score || 0) + (vote.value === 1 ? 1 : 0),
)

const displayedDownvotes = computed(
  () => Number(props.downvotes || 0) + (vote.value === -1 ? 1 : 0),
)
...
<template>
  <div class="post-vote-control" :aria-label="label">
    ...
    >
      ...
      <span class="post-vote-button__label">추천</span>
      <strong class="post-vote-button__count">{{ displayedUpvotes }}</strong>
    </button>
    ...
    >
      ...
      <span class="post-vote-button__label">비추천</span>
      <strong class="post-vote-button__count">{{ displayedDownvotes }}</strong>
    </button>
  </div>
</template>
